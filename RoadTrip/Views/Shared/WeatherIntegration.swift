    ...
    func analyzeWeatherForTrip(_ trip: Trip) async {
        ...
        for day in trip.days {
            ...
            if !day.startLocation.isEmpty {
                if let weather = try? await WeatherService.shared.fetchWeather(for: day.startLocation, date: day.date) {
            ...
            for activity in day.activities {
                if let weather = try? await WeatherService.shared.fetchWeather(for: activity.location, date: day.date) {
    ...
    func loadForecast() async {
        ...
        for i in 0..<7 {
            if let forecastDate = Calendar.current.date(byAdding: .day, value: i, to: day.date) {
                if let weather = try? await weatherService.fetchWeather(for: day.startLocation, date: forecastDate) {
